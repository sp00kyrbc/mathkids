# 02-arithmetic-engine.md — Silnik arytmetyczny (Python)

## Kontekst
Ten plik tworzy serce aplikacji — Python'owy silnik który:
1. Generuje zadania pisemne
2. Oblicza każdy krok algorytmu (nie tylko wynik!)
3. Waliduje odpowiedzi dziecka krok po kroku
4. Zwraca strukturę danych opisującą KAŻDY krok działania pisemnego

**KLUCZOWE**: Aplikacja uczy ALGORYTMU, nie tylko wyniku. Każde działanie musi być rozbite na kroki dokładnie tak jak uczy się w polskiej szkole podstawowej.

---

## Zasady działań pisemnych (polska szkoła podstawowa)

### Dodawanie pod kreską
```
    1 1       ← przeniesienia (carry)
    4 5 6
  + 2 8 7
  -------
    7 4 3
```
Algorytm: od prawej, kolumna po kolumnie. Jeśli suma ≥ 10: zapisz jedności, przenieś dziesiątki na następną kolumnę.
...
```
  ...
  -------
    2 4 5
```
Algorytm: od prawej, kolumna po kolumnie. Jeśli odjemna < odjemnik: pożycz 10 z lewej kolumny (zapisz małą 1 nad odjemną, zmniejsz lewą cyfrę o 1).
...
```
    ...
    ×   2 3
    -------
    1 0 3 5   ← 345 × 3 (jedności)
  + 6 9 0 0   ← 345 × 20 (dziesiątki, przesunięte o 1)
  ---------
    7 9 3 5
```
Algorytm: mnóż przez każdą cyfrę mnożnika (od prawej), każdy wynik przesunięty o odpowiednią liczbę miejsc. Na końcu dodaj wyniki cząstkowe.

### Dzielenie pod kreską (polska metoda)
```
  1 2 6 | 6
  1 2   |------
  ------| 2 1
    ...
    ----|
      0 |
```
Algorytm: bierz cyfry dzielnej od lewej (tyle ile potrzeba by podzielić), szacuj cyfrę wyniku, odejmij, dopisz następną cyfrę reszty.

---

## Utwórz plik: `backend/src/python/arithmetic_engine.py`
...
## Utwórz plik testowy: `backend/src/python/test_engine.py`
...
Uruchom test: `cd backend/src/python && python3 test_engine.py`

---

## ✅ Weryfikacja po tym kroku

- [ ] `backend/src/python/arithmetic_engine.py` istnieje
- [ ] `python3 test_engine.py` nie wyrzuca błędów
- [ ] Wszystkie 4 operacje generują zadania z krokami
- [ ] Każdy krok ma: `description`, `result_digit`, `hint`

Następny krok: `docs/03-backend-api.md`
